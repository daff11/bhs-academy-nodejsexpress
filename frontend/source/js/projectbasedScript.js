...
function handleJoinNowProjectBased(programId) {
...
function getAllProjectBased() {
    fetch('/program/projectbasedall') 
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            const projectbasedList = document.querySelector('.row.justify-content-center'); 
            
            projectbasedList.innerHTML = ''; 
            ...
            data.data.forEach(item => {
                ...
                courseItem.innerHTML = `
                    <div class="card p-0 border-primary rounded-0 hover-shadow">
                        <img class="card-img-top rounded-0 img-course-thumb" src="${item.gambar}" alt="projectbased thumb">
                        <div class="card-body">
                            ...
                            <a href="/program/projectbased/single?id=${item.id}">
                                <h5 class="card-title">${item.nama_program}</h4>
                            </a>
                            <p class="card-text mb-4" style="text-align: left">${truncateText(item.detail, 7)}</p>
                            <ul class="list-inline">
                                <li class="list-inline-item mb-20-m"><a onclick="handleJoinNowProjectBased('${item.id}')" class="btn btn-primary btn-sm">Join now</a></li>
                ...
                projectbasedList.appendChild(courseItem);
            });
        })
        .catch(error => console.error('Fetch error:', error));
}

async function get3ProjectBased() {
    ...
    fetch('/program/projectbased3')
        .then(response => response.json())
        .then(data => {
            const projectbasedList = document.querySelector('.projectbased');
            projectbasedList.innerHTML = '';
            ...
            data.data.forEach(item => {
                ...
                let buttonOnClick = `handleJoinNowProjectBased(${item.id})`;
                ...
                courseItem.innerHTML = `
                    <div class="card p-0 border-primary rounded-0 hover-shadow">
                        ...
                        <div class="card-body">
                            <ul class="list-inline mb-2">
                                <li class="list-inline-item bg-third padds"><a class="text-primary" href="/program/projectbasedall"><strong>${item.type.nama}</strong></a></li>
...
function pageProjectBasedById() {
    fetch('/program/projectbased/single/:id')
}

function pageProjectBasedPaymentById() {
    fetch('/projectbased/payment-details/:id')
}
...
async function getProjectBasedById() {
    ...
    try {
        ...
        const response = await fetch(`/program/projectbasedid/${id}`);
        ...
        // Isi elemen HTML di projectbased-single.html dengan data yang diterima
        ...
        const joinNowButtons = document.querySelectorAll(".join-now-projectbased");
        joinNowButtons.forEach(button => {
            if (isCompleted) {
                ...
                button.setAttribute("onclick", `handleJoinNowProjectBased('${program.id}')`);
            }
        });
...
function getProjectBasedPaymentById() {
    const id = getIdFromUrl();
    if (!id) {
        console.error("ID tidak ditemukan di URL");
        return;
    ...
    fetch(`/program/projectbasedid/${id}`) 
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok ' + response.statusText);
            }
