...
function handleJoinNowShortCourses(programId) {
    ...
    .then(data => {
        console.log(data);
        if (data.loggedIn) {
            window.location.href = `/program/coorporate/payment-details?id=${programId}`;
        }
    })
    .catch(error => console.error('Error in handleJoinNow:', error));        
}

function getAllShortCourses() {
    fetch('/program/shortcoursesall') 
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            const shortcoursesList = document.querySelector('.row.justify-content-center'); 
            
            shortcoursesList.innerHTML = ''; 
            ...
            data.data.forEach(item => {
                ...
                courseItem.innerHTML = `
                    <div class="card p-0 border-primary rounded-0 hover-shadow">
                        <img class="card-img-top rounded-0 img-course-thumb" src="${item.gambar}" alt="shortcourses thumb">
                        <div class="card-body">
                            <ul class="list-inline mb-2">
                                <li class="list-inline-item bg-third padds"><a class="text-primary" href="short-course.html"><strong>${item.type.nama}</strong> </a></li>
                                <li class="list-inline-item bg-third padds"><i class="ti-alarm-clock mr-1 text-primary"></i><strong class="text-primary">${item.durasi_jamhari} Jam/Hari</strong> </li>
                            </ul>
                            <a href="/program/shortcourses/single?id=${item.id}">
                                <h5 class="card-title">${item.nama_program}</h4>
                            </a>
                            <p class="card-text mb-4" style="text-align: left">${truncateText(item.detail, 7)}</p>
                            <ul class="list-inline">
                                <li class="list-inline-item mb-20-m"><a onclick="handleJoinNowShortCourses('${item.id}')" class="btn btn-primary btn-sm">Join now</a></li>
                                <li class="list-inline-item"><h4 class="card-title">Rp${item.harga.toLocaleString('id-ID')}</h4></li> 
                            </ul>
                        </div> 
                    </div>
                `;
                
                shortcoursesList.appendChild(courseItem);
            });
        })
        .catch(error => console.error('Fetch error:', error));
}

async function get3ShortCourses() {
    ...
    fetch('/program/shortcourses3')
        .then(response => response.json())
        .then(data => {
            const shortcoursesList = document.querySelector('.shortcourses');
            shortcoursesList.innerHTML = '';
            ...
            data.data.forEach(item => {
                ...
                let buttonOnClick = `handleJoinNowShortCourses(${item.id})`;
                ...
                courseItem.innerHTML = `
                    <div class="card p-0 border-primary rounded-0 hover-shadow">
                        ...
                        <div class="card-body">
                            <ul class="list-inline mb-2">
                                <li class="list-inline-item bg-third padds"><a class="text-primary" href="/program/shortcoursesall"><strong>${item.type.nama}</strong></a></li>
...
function pageShortCoursesById() {
    fetch('/program/shortcourses/single/:id')
}

function pageShortCoursesPaymentById() {
    fetch('/shortcourses/payment-details/:id')
}
...
async function getShortCoursesById() {
    ...
    try {
        ...
        const response = await fetch(`/program/shortcoursesid/${id}`);
        ...
        // Isi elemen HTML di shortcourses-single.html dengan data yang diterima
        ...
        const joinNowButtons = document.querySelectorAll(".join-now-shortcourses");
        joinNowButtons.forEach(button => {
            if (isCompleted) {
                ...
                button.setAttribute("onclick", `handleJoinNowShortCourses('${program.id}')`);
            }
        });
...
function getShortCoursesPaymentById() {
    const id = getIdFromUrl();
    if (!id) {
        console.error("ID tidak ditemukan di URL");
        return;
    ...
    fetch(`/program/shortcoursesid/${id}`) 
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok ' + response.statusText);
            }
